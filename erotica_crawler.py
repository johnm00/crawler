...
def get_anti_links(html):
	tuples = []
	soup = bs(html, "lxml")
	uebersicht = soup.find_all("td", {"class" : "mcol"})
	for td in uebersicht:
		a = td.find_all("a")[0]
		a_link = a['href']
		a_text = a.text
...
def get_overview_links(html):
	tuples = []
	soup = bs(html, "lxml")
	uebersicht = soup.find_all("div", {"class" : "b-sl-item-r w-34t"})
	for td in uebersicht:
		a = td.find_all("a")[0]
		a_link = a['href']
		a_text = a.text
		...
		#print(rating)
		tuples.append([a_text, a_link, rating])
...
def retrieve_text(link):
    ...
    html = load_url(link)
    soup = bs(html, "lxml")
    uebersicht = soup.find("div", {"class" : "b-story-body-x x-r15"}).find_all('p')
    for p in uebersicht:
        p_text = p.text
        txt += ' ' + p_text
    return txt

def save_erotica(title, text, rating, i):
    splittitle = title.split()
    filepath = 'couplings/' + str(i) + '_' + str('_'.join(splittitle)) + '_' + str(rating) + '.txt'
    try:
...
if __name__ == "__main__":
    ...
    for i in range(1,380):
        ...
        literotica_link = 'https://www.literotica.com/c/erotic-couplings/' + str(i) + '-page'
        ...
        #print(links)
        for title, link, rating in links:
            print(counter, title, link, rating)
            if rating > 4.0:
                counter += 1
                try:
                    ...
                    #print(text)
                    print('SAVING')
                    save_erotica(title, text, rating, counter)
                except ValueError:
                    continue
